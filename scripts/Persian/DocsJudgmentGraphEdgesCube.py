...
from doc.models import JudgmentGraphType, Judgment, JudgmentGraphNodesCube, JudgmentGraphEdgesCube
...
def apply(folder_name, Country):
    JudgmentGraphEdgesCube.objects.filter(country_id=Country).delete()
    ...
    for i in range(type_list.count()-1):
        ...
        for j in range(i+1, type_list.count()):
            ...
            edges_List = Judgment.objects.filter(document_id__country_id=Country)\
            ...
            JudgmentGraphEdgesCube.objects.create(country_id=Country, src_type_id=src_type_id, target_type_id=target_type_id, edges=edges_list)
