...
def apply(folder_name, Country,host_url,panel_name):

    general_panels =  Template_Panels_Info.objects.all()
    
    if panel_name != "All":
        general_panels = general_panels.filter(panel_name = panel_name)

    for panel in general_panels:
        print(panel.panel_name + ":")
        
        keywords_info = panel.keywords_info
        keyword_list = keywords_info['keyword_list']

        create_FullData_CUBE(Country,panel,keyword_list)

        create_ChartData_CUBE(Country,panel)

        create_TableData_CUBE(Country,host_url,panel)

        print("---------------------------------------------")

def create_FullData_CUBE(Country,panel,keywords_template):
    start_t = time.time()
    CUBE_Template_FullData.objects.filter(country_id=Country,panel_id = panel).delete()
    Create_List = []
    batch_size = 10000
    document_list = Document.objects.filter(country_id = Country).values('id')
    result_doc_ids = DocumentParagraphs.objects.filter(
        reduce(operator.or_, (Q(text__icontains = kw['searched_template']) for kw in keywords_template)),
...
def create_ChartData_CUBE(Country,panel):

    batch_size = 10000
    Create_List = []

    start_t = time.time()
    CUBE_Template_ChartData.objects.filter(country_id=Country,panel_id = panel).delete()
    ...
    for subject in subject_list:
        if subject == 'همه':
            doc_list = CUBE_Template_FullData.objects.filter(country_id = Country,panel_id = panel)
        else:
...
def create_TableData_CUBE(Country,host_url,panel):
    start_t = time.time()

    batch_size = 10000


    CUBE_Template_TableData.objects.filter(country_id=Country,panel_id = panel).delete()
    ...
    for subject in subject_list:
        ...
        if subject == 'همه':
            doc_list = CUBE_Template_FullData.objects.filter(
                country_id = Country,panel_id = panel).order_by('-keyword_count')
        else:
            doc_list = CUBE_Template_FullData.objects.filter(
                country_id = Country,
                panel_id = panel,
...
def GetKeywordsActors_ByRole(doc_id,actor_role,keywords_template):
    ...
    document_actors = DocumentActor.objects.filter(
        reduce(operator.or_, (Q(paragraph_id__text__icontains = kw['searched_template']) for kw in keywords_template)),
