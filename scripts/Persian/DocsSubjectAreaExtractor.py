...
def calculate_entropy(subject_dict):
    score_list = list(subject_dict.values()) 
    s = sum(score_list)
    if s>0:
        probabilities = [n/s for n in score_list]
    else:
        probabilities = [0] * len(score_list)
    etp = 0
...
def apply(folder_name, Country):
    ...
    DocumentSubjectSubAreaKeywords.objects.filter(document_id__country_id=Country).delete()
    DocumentSubjectSubArea.objects.filter(document_id__country_id=Country).delete()

    Subject_Keyword_Dict = {}
    subject_list = SubjectSubArea.objects.filter(subject_area_id__language=Country.language).values('name','id','subject_area_id__id','subject_area_id__name')

    sub_area_to_area_dict = {}
    for subject in subject_list:
        sub_area_to_area_dict[subject['id']] = (subject['subject_area_id__id'],subject['subject_area_id__name'])
    ...
    for subject in subject_list:
        ...
        Subject_Keyword_Dict[(subject_name, subject_id)] = text_title_keyword_list
    ...
    for document_id, subject_score_dict in Document_Subject_Score.items():
        ...
        if subject_score_dict[doc_subject_id] > 0:
            ...
            entropy = calculate_entropy(subject_score_dict)
            Document.objects.filter(id=document_id).update(subject_sub_area_id_id=doc_subject_id,
                                                           ...
                                                           subject_area_id_id=sub_area_to_area_dict[doc_subject_id][0],
                                                           subject_area_name=sub_area_to_area_dict[doc_subject_id][1],
                                                           subject_sub_area_entropy=entropy)
